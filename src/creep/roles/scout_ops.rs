...
pub fn try_register_scout_target(
    creep_name: &String,
...
fn find_scout_target(
    ...
    if !unscouted_targets.is_empty() {
        return Some(choose_unscouted_target(
            creep_name,
            room_name,
            game_state,
            memory,
...
fn choose_unscouted_target(
    creep_name: &String,
    room_name: &RoomName,
    game_state: &mut GameState,
    memory: &GameMemory,
    unscouted_targets: Vec<RoomName>,
) -> RoomName {
    let creep_memory = memory.creeps.get(creep_name).unwrap();
...
fn scout(
    ...
    // By this point the room sould have a memory
    let room_memory = memory.rooms.get(room_name).unwrap();

    let room_from = {
        let creep_memory = memory.creeps.get(creep_name).unwrap();

        creep_memory.room_from
    };

    match room_memory.room_type {
        StaticRoomType::Claimable => match memory.remotes.get(room_name) {
            Some(remote_memory) => {}
            // If there is no remote memory, try to construct one
            None => {
                room_ops::try_add_remote(room_name, &room_from, game_state, memory);
            }
        },
        _ => {}
    };
